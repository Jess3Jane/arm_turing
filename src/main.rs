...
fn exec(code: &Vec<u8>, start_addr: u64, data_addr: usize, data_len: usize, exec_len: usize) -> Result::<CpuARM, unicorn::Error> {
    ...
    emu.add_code_hook(unicorn::unicorn_const::CodeHookType::CODE, start_addr, start_addr, move |emu, _, _| {
        print_data(&emu.mem_read(data_addr as u64 + 1, data_len).unwrap());
    });
    emu.emu_start(start_addr, start_addr + code.len() as u64, 10 * unicorn::SECOND_SCALE, exec_len);
    Ok(emu)
}
...
fn preprocess(file: &str, output: &str) {
    ...
    File::create(output).unwrap().write_fmt(format_args!("{}", expanded));
}

fn assemble(data: &Vec<u8>, output: &str) -> usize {
    let mut process_rule = String::new();
    File::open("templates/process_rule.s").unwrap().read_to_string(&mut process_rule);
    let mut copy = String::new();
    File::open("templates/copy.s").unwrap().read_to_string(&mut copy);
    let mut rule = String::new();
    File::open("templates/rule.s").unwrap().read_to_string(&mut rule);
    ...
    code.write_all(b"0, ");
    code.write_all(data.iter().map(|i| format!("{}", i)).collect::<Vec<String>>().join(",").as_str().as_bytes());
    code.write_all(b", 0");
    code.write_all(b"\n\n");

    // tmp vector
    code.write_all(b"tmp:\n.byte ");
    code.write_all((0..data.len()+2).map(|_| String::from("0")).collect::<Vec<String>>().join(",").as_str().as_bytes());
    code.write_all(b"\n\n");
...
fn print_data(data: &[u8]) {
    println!("|{}|", data.iter().map(|v| { match v { 0 => ' ', 1 => '#', _ => '~' } }).collect::<String>());
}
